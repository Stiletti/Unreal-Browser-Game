...
if (AudioContext) {
  ...
  ue4_hacks.panner.__proto__.setVelocity =
    ue4_hacks.panner.__proto__.setVelocity || function () {};
}
...
var Module = {
  ...
  UE4_indexedDBName: "UE4_assetDatabase_html", // this should be an ascii ID string without special characters that is unique to the project that is being packaged
  UE4_indexedDBVersion: 202402082118, // Bump this number to invalidate existing IDB storages in browsers.
};

// ================================================================================
// *** HTML5 UE4 ***

Module.arguments = ["../../../html/html.uproject", "-stdout"];
...
if (location.host != "" && location.search.indexOf("cookonthefly") != -1) {
  Module.arguments.push(
    "'-filehostIp=" + location.protocol + "//" + location.host + "'"
  );
}
...
window.onerror = function (e) {
  ...
  if (e.toLowerCase().indexOf("memory") != -1) {
    e += "<br>";
    if (!heuristic64BitBrowser)
      e += " Try running in a 64-bit browser to resolve.";
  }
  showErrorDialog(e);
};
...
function heuristicIs64Bit(type) {
  function contains(str, substrList) {
    for (var i in substrList) if (str.indexOf(substrList[i]) != -1) return true;
    return false;
  }
  var ua = (
    navigator.userAgent +
...
function detectWebGL() {
  var canvas = targetOffscreenCanvas
  ...
  function testError(e) {
    Module["webGLErrorReason"] = e.statusMessage;
  }
  canvas.addEventListener("webglcontextcreationerror", testError, false);
  try {
    for (
      var failIfMajorPerformanceCaveat = 1;
      ...
      for (var i in names) {
        try {
          ...
          Module["webGLErrorReason"] = e.toString();
        }
      }
    }
  } finally {
...
function resizeCanvas(aboutToEnterFullscreen) {
  ...
  if (aboutToEnterFullscreen && !aboutToEnterFullscreen.type) {
    // UE4 engine is calling this function right before entering fullscreen?
  ...
  var cssHeight = Math.max(
    minimumCanvasHeightCssPixels,
    canvasRect.bottom - canvasRect.top,
    window.innerHeight * minimumCanvasHeightFractionOfBrowserWindowHeight
  );
  ...
  if (canvasWindowedScaleMode == 3 /*NONE*/) {
    ...
    var newRenderTargetWidth = canvasWindowedUseHighDpi
      ? cssWidth * window.devicePixelRatio
      : cssWidth;
    var newRenderTargetHeight = canvasWindowedUseHighDpi
    ...
    if (canvasWindowedScaleMode == 2 /*ASPECT*/) {
      if (
        cssWidth * canvasAspectRatioHeight >
        canvasAspectRatioWidth * cssHeight
      ) {
        newRenderTargetWidth =
          (newRenderTargetHeight * canvasAspectRatioWidth) /
          canvasAspectRatioHeight;
      } else {
        newRenderTargetHeight =
          (newRenderTargetWidth * canvasAspectRatioHeight) /
          canvasAspectRatioWidth;
      }
    }
  ...
  cssWidth = canvasWindowedUseHighDpi
    ? newRenderTargetWidth / window.devicePixelRatio
    : newRenderTargetWidth;
  cssHeight = canvasWindowedUseHighDpi
    ? newRenderTargetHeight / window.devicePixelRatio
    : newRenderTargetHeight;
...
Module["UE4_mouseEvent"] = function (
  eventType,
  x,
  y,
  button,
...
Module["UE4_fullscreenCanvasResizeMode"] = canvasWindowedUseHighDpi
  ? 2 /*HIDPI*/
  : 1 /*Standard DPI*/;
...
var enableWriteToIndexedDB =
  enableReadFromIndexedDB && location.search.indexOf("noidbwrite") == -1;
enableReadFromIndexedDB = false;
enableWriteToIndexedDB = false;

// if (!enableReadFromIndexedDB)
//   showWarningRibbon("Running with IndexedDB access disabled.");
// else if (!enableWriteToIndexedDB)
//   showWarningRibbon("Running in read-only IndexedDB access mode.");

function getIDBRequestErrorString(req) {
  try {
    return req.error
      ? "IndexedDB " + req.error.name + ": " + req.error.message
      : req.result;
  } catch (ex) {
    return null;
  }
...
function reportDataBytesStoredInIndexedDB(deltaBytes) {
  if (deltaBytes === null) Module["dataBytesStoredInIndexedDB"] = 0;
  // call with deltaBytes == null to report that DB was cleared.
  else Module["dataBytesStoredInIndexedDB"] += deltaBytes;
  document.getElementById("clear_indexeddb").innerText =
    "Clear IndexedDB (" +
    formatBytes(Module["dataBytesStoredInIndexedDB"]) +
    ")";
}
...
function deleteIndexedDBStorage(dbName, onsuccess, onerror, onblocked) {
  ...
  req.onsuccess = function () {
    console.log("Deleted IndexedDB storage " + dbName + "!");
    reportDataBytesStoredInIndexedDB(null);
    if (onsuccess) onsuccess();
  };
  req.onerror = function (evt) {
    var errorString = getIDBRequestErrorString(req);
    console.error(
      "Failed to delete IndexedDB storage " + dbName + ", " + errorString
    );
    evt.preventDefault();
  ...
  req.onblocked = function (evt) {
    var errorString = getIDBRequestErrorString(req);
    console.error(
      "Failed to delete IndexedDB storage " +
        dbName +
        ", DB was blocked! " +
        errorString
    );
...
function storeToIndexedDB(db, key, value) {
  return new Promise(function (resolve, reject) {
    if (!enableWriteToIndexedDB)
      return reject(
        'storeToIndexedDB: IndexedDB writes disabled by "?noidbwrite" option'
      );
    function fail(e) {
      console.error(
        "Failed to store file " + key + " to IndexedDB storage! error: " + e
      );
      if (!Module["idberrorShown"]) {
        showWarningRibbon(
          "Failed to store file " + key + " to IndexedDB, error: " + e
        );
    ...
    if (location.protocol.indexOf("file") != -1)
      return reject("Loading via file://, skipping caching to IndexedDB");
    ...
    try {
      ...
      putRequest.onsuccess = function (evt) {
        if (value.byteLength || value.length)
          reportDataBytesStoredInIndexedDB(
            value.size || value.byteLength || value.length
          );
        resolve(key);
      };
      putRequest.onerror = function (evt) {
        var errorString =
          getIDBRequestErrorString(putRequest) ||
          "IndexedDB request error: " + evt;
        evt.preventDefault();
        fail(errorString);
      };
    } catch (e) {
...
function fetchFromIndexedDB(db, key) {
  return new Promise(function (resolve, reject) {
    if (!enableReadFromIndexedDB)
      return reject(
        'fetchFromIndexedDB: IndexedDB reads disabled by "?noidbread" option'
      );
    function fail(e) {
      console.error(
        "Failed to read file " + key + " from IndexedDB storage! error:"
      );
      console.error(e);
      if (!Module["idberrorShown"]) {
        showWarningRibbon(
          "Failed to read file " + key + " from IndexedDB, error: " + e
        );
        Module["idberrorShown"] = true;
      }
      return reject(e);
    ...
    try {
      ...
      getRequest.onerror = function (evt) {
        var errorString =
          getIDBRequestErrorString(getRequest) ||
          "IndexedDB.get request error: " + evt;
        evt.preventDefault();
        fail(errorString);
      };
    } catch (e) {
            ...
            /*return*/ storeToIndexedDB(db, url, data)
              ...
              .catch(function (error) {
                if (enableReadFromIndexedDB || enableWriteToIndexedDB) {
                  console.error(
                    "Failed to store download to IndexedDB! " + error
                  );
                }
...
function openIndexedDB(dbName, dbVersion) {
  return new Promise(function (resolve, reject) {
    if (!enableReadFromIndexedDB)
      return reject('openIndexedDB: IndexedDB disabled by "?noidbread" option');
    try {
      var idb =
        window.indexedDB ||
        window.mozIndexedDB ||
    ...
    openRequest.onerror = function (evt) {
      var errorString =
        getIDBRequestErrorString(openRequest) ||
        "IndexedDB request error: " + evt;
      evt.preventDefault();
      reject(errorString);
    };
...
Module.locateFile = function (name) {
  ...
  var isFileProtocol =
    name.indexOf("file://") != -1 || location.protocol.indexOf("file") != -1;
  if (isFileProtocol) {
...
Module.getPreloadedPackage = function (remotePackageName, remotePackageSize) {
  return Module["preloadedPackages"]
    ? Module["preloadedPackages"][remotePackageName]
    : null;
};
...
Module["instantiateWasm"] = function (info, receiveInstance) {
  Module["wasmDownloadAction"]
    .then(function (downloadResults) {
      taskProgress(TASK_COMPILING);
      var wasmInstantiate = WebAssembly.instantiate(
        downloadResults.wasmModule || new Uint8Array(downloadResults.wasmBytes),
        info
      );
      return wasmInstantiate.then(function (output) {
        ...
        if (!downloadResults.fromIndexedDB) {
          storeToIndexedDB(downloadResults.db, "wasmModule", module).catch(
            function () {
    ...
    .catch(function (error) {
      $("#mainarea").empty();
      $("#mainarea").append(
        '<div class="alert alert-danger centered-axis-xy" style ="min-height: 10pt" role="alert">WebAssembly instantiation failed: <br> ' +
          error +
          "</div></div>"
      );
    });
...
function compileShadersFromJson(jsonData) {
  var shaderPrograms = [];
  if (jsonData instanceof ArrayBuffer)
    jsonData = new TextDecoder("utf-8").decode(new DataView(jsonData));
  ...
  var promise = new Promise(function (resolve, reject) {
    ...
    function buildProgram() {
      ...
      taskProgress(TASK_SHADERS, {
        current: nextProgramToBuild,
        total: shaderPrograms.length,
      });
      ...
      if (compileLog)
        console.error("Compiling vertex shader: " + lineNumberize(p.vs));
      if (!success) console.error("Vertex shader compilation failed!");
      if (compileLog) console.error("Compilation log: " + compileLog);
      if (!success)
        return reject("Vertex shader compilation failed: " + compileLog);
      ...
      if (compileLog) compileLog = compileLog.trim();
      if (compileLog)
        console.error("Compiling fragment shader: " + lineNumberize(p.fs));
      if (!success) console.error("Fragment shader compilation failed!");
      if (compileLog) console.error("Compilation log: " + compileLog);
      if (!success)
        return reject("Fragment shader compilation failed: " + compileLog);
      ...
      for (var i = 0; i < numUniforms; ++i) {
        ...
        if (Module["precompiledUniforms"].length != Module["glIDCounter"])
          throw (
            "uniforms array not in sync! " +
            Module["precompiledUniforms"].length +
            ", " +
            Module["glIDCounter"]
          );
...
function taskProgress(taskId, progress) {
  ...
  if (!l) {
    ...
    l.innerHTML =
      '<span id="icon_' +
      taskId +
      '" class="glyphicon glyphicon-refresh glyphicon-spin"></span>  <span id="load_' +
      taskId +
      '"></span>';
    tasks.appendChild(l);
  ...
  if (progress && progress.total) {
    text +=
      ": " +
      (progress.currentShow || progress.current) +
      "/" +
      (progress.totalShow || progress.total) +
      " (" +
      ((progress.current * 100) / progress.total).toFixed(0) +
      "%)";
  } else {
    text += "...";
  }
...
function taskFinished(taskId, error) {
  ...
  if (l && icon) {
    ...
    if (!error) {
      l.innerHTML =
        loadTasks[taskId] + " (" + (totalTime / 1000).toFixed(2) + "s)";
...
function reportDownloadProgress(url, downloadedBytes, totalBytes, finished) {
  ...
  for (var i in Module["assetDownloadProgress"]) {
    ...
    aggregated.finished =
      aggregated.finished && Module["assetDownloadProgress"][i].finished;
  }
...
function download(url, responseType) {
  return new Promise(function (resolve, reject) {
    ...
    xhr.onerror = function (e) {
      var isFileProtocol =
        url.indexOf("file://") == 0 || location.protocol.indexOf("file") != -1;
      if (isFileProtocol)
        taskFinished(
          TASK_DOWNLOADING,
          "HTTP error " +
            ...
            " on file " +
            url +
            '<br>Try using a web server to avoid loading via a "file://" URL.'
        );
      // Convert the most common source of errors to a more friendly message format.
      else
        taskFinished(
          TASK_DOWNLOADING,
          "HTTP error " +
        ...
        if (url.endsWith("gz") && (xhr.status == 0 || xhr.status == 200)) {
          if (xhr.getResponseHeader("Content-Encoding") != "gzip") {
            ...
            taskFinished(
              ...
              "Downloaded a compressed file " +
                url +
                ' without the necessary HTTP response header "Content-Encoding: gzip" specified!<br>Please configure gzip compression on this asset on the web server to serve gzipped assets!'
            );
            xhr.onload =
              xhr.onprogress =
              xhr.onerror =
              xhr.onreadystatechange =
                null; // Abandon tracking events from this XHR further.
            xhr.abort();
            return reject({
              status: 406,
          ...
          if (
            ...
            taskFinished(
              ...
              "Downloaded a compressed file " +
                url +
                ' with incorrect HTTP response header "Content-Type: ' +
                xhr.getResponseHeader("Content-Type") +
                '"!<br>Please set the MIME type of the asset to "' +
                expectedMimeType(url) +
                '".'
            );
            xhr.onload =
              xhr.onprogress =
...
function showErrorDialog(errorText) {
  if (errorText.indexOf("SyntaxError: ") != -1) {
    // this may be due to caching issue -- otherwise, compile time would have caught this
    errorText =
      "NOTE: attempting to flush cache and force reload...<br>Please standby...";
    setTimeout(function () {
  ...
  if (existingErrorDialog) {
    ...
    $("#mainarea").empty();
    $("#mainarea").append(
      '<div class="alert alert-danger centered-axis-xy" style ="min-height: 10pt" role="alert" id="errorDialog">' +
        errorText +
        "</div></div>"
    );
  }
...
function showWarningRibbon(warningText) {
  ...
  if (existingWarningDialog) {
    ...
    $("#buttonrow").prepend(
      '<div class="alert alert-warning centered-axis-x" role="warning" id="warningDialog" style="padding-top:5px; padding-bottom: 5px">' +
        warningText +
        "</div></div>"
    );
  }
...
function addScriptToDom(scriptCode) {
  return new Promise(function (resolve, reject) {
    ...
    var blob =
      scriptCode instanceof Blob
        ? scriptCode
        : new Blob([scriptCode], { type: "text/javascript" });
...
$(document).ready(function () {
  ...
  if (!supportsWasm) {
    showErrorDialog(
      "Your browser does not support WebAssembly. Please try updating to latest 64-bit browser that supports WebAssembly.<br>Current user agent: " +
        navigator.userAgent
    );
    return;
  }
  ...
  if (
    !Module["WEBGL_VERSION"] ||
    Module["WEBGL_VERSION"] < requiredWebGLVersion
  ) {
    showErrorDialog(
      "Your browser does not support WebGL " +
        requiredWebGLVersion +
        "<br>Error reason: " +
        (Module["webGLErrorReason"] || "Unknown") +
        ". Try updating your browser and/or graphics card drivers.<br>Current renderer: " +
        getGpuInfo()
    );
    return;
  }
  ...
  if (Module["WEBGL_VERSION"] < 2 && !explicitlyUseWebGL1) {
    if (shouldBrowserSupportWebGL2()) {
      showWarningRibbon(
        "Your GPU does not support WebGL 2. This affects graphics performance and quality. Please try updating your graphics driver and/or browser to latest version.<br>Error reason: " +
          (Module["webGLErrorReason"] || "Unknown") +
          "<br>Current renderer: " +
          getGpuInfo()
      );
      ...
      showWarningRibbon(
        "The current browser does not support WebGL 2. This affects graphics performance and quality.<br>Please try updating your browser (and/or video drivers).  NOTE: old hardware might have been blacklisted by this browser -- you may need to use a different browser.<br>Error reason: " +
          (Module["webGLErrorReason"] || "Unknown") +
          "<br>Current renderer: " +
          getGpuInfo()
      );
  ...
  var supportedWebGLExtensions =
    Module["preinitializedWebGLContext"].getSupportedExtensions();
  if (Module["WEBGL_VERSION"] >= 2)
    supportedWebGLExtensions = supportedWebGLExtensions.concat(
      webGLExtensionsInCoreWebGL2
    );
  ...
  for (var i in requiredWebGLExtensions) {
    if (supportedWebGLExtensions.indexOf(requiredWebGLExtensions[i]) == -1) {
      showErrorDialog(
        "Your browser does not support WebGL extension " +
          requiredWebGLExtensions[i] +
          ", which is required to run this page!"
      );
    }
  }
  ...
  var preferredToHaveWebGLExtensions = [
    // The following are core in WebGL 2:
  ...
  for (var i in preferredToHaveWebGLExtensions) {
    if (
      supportedWebGLExtensions.indexOf(preferredToHaveWebGLExtensions[i]) == -1
    ) {
  ...
  if (unsupportedWebGLExtensions.length > 1) {
    showWarningRibbon(
      "Your browser or graphics card does not support the following WebGL extensions: " +
        unsupportedWebGLExtensions.join(", ") +
        ". This can impact UE4 graphics performance and quality."
    );
  } else if (unsupportedWebGLExtensions.length == 1) {
    showWarningRibbon(
      "Your browser or graphics card does not support the WebGL extension " +
        unsupportedWebGLExtensions[0] +
        ". This can impact UE4 graphics performance and quality."
    );
  }
  ...
  if (!heuristicIs64Bit("browser")) {
    if (heuristicIs64Bit("os")) {
      showWarningRibbon(
        "It looks like you are running a 32-bit browser on a 64-bit operating system. This can dramatically affect performance and risk running out of memory on large applications. Try updating to a 64-bit browser for an optimized experience."
      );
    } else {
      showWarningRibbon(
        "It looks like your computer hardware is 32-bit. This can dramatically affect performance."
      );
    }
  }
  ...
  function withIndexedDB(db) {
    ...
    // ----------------------------------------
    // WASM
    var mainCompiledCode = fetchFromIndexedDB(db, "wasmModule")
      ...
      .catch(function () {
        return download(
          Module.locateFile("html-HTML5-Shipping.wasm"),
          "arraybuffer"
    ...
    // ----------------------------------------
    // MAIN JS
    var mainJsDownload = fetchOrDownloadAndStore(
      db,
      Module.locateFile("html-HTML5-Shipping.js"),
      "blob"
    ).then(function (data) {
    ...
    // ----------------------------------------
    // MORE JS
    var dataJsDownload = fetchOrDownloadAndStore(
      db,
      Module.locateFile("html-HTML5-Shipping.data.js")
    );
    var utilityJsDownload = fetchOrDownloadAndStore(
      db,
    ...
    var dataDownload =
      /* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('html-HTML5-Shipping.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('html-HTML5-Shipping.data')] = dataArrayBuffer;
...
*/
      ...
      fetchOrDownloadAndStore(
        db,
        Module.locateFile("html-HTML5-Shipping.data"),
        "arraybuffer"
      ).then(function (dataArrayBuffer) {
        Module["preloadedPackages"] = {};
        Module["preloadedPackages"][
          Module.locateFile("html-HTML5-Shipping.data")
        ] = dataArrayBuffer;
    ...
    if (precompileShaders) {
      var compileShaders = fetchOrDownloadAndStore(
        db,
        Module.locateFile("shaders.json"),
        "arraybuffer"
      ).then(function (json) {
        return compileShadersFromJson(json).catch(function (error) {
          taskFinished(
            TASK_SHADERS,
            error + "<br>Current renderer: " + getGpuInfo()
          );
  ...
  // GO !
  openIndexedDB(
    Module["UE4_indexedDBName"],
    Module["UE4_indexedDBVersion"] || 1
  )
    .then(withIndexedDB)
    .catch(function (e) {
      if (enableReadFromIndexedDB || enableWriteToIndexedDB) {
